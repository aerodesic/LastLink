...
static const char *TAG = "flash_dev";
...
bool init_flash_file_system(void)
{
    ...
    if (flash != NULL) {
        ...
        if (!mount_fatfs(partition_label)) {
            ESP_LOGI(TAG, "Unable to mount partition %s", partition_label);
	} else {
...
static esp_flash_t* init_ext_flash(void)
{
    const spi_bus_config_t bus_config = {
        .mosi_io_num = CONFIG_MESHNET_FLASH_MOSI,
        .miso_io_num = CONFIG_MESHNET_FLASH_MISO,
        .sclk_io_num = CONFIG_MESHNET_FLASH_SCK,
        .quadwp_io_num = CONFIG_MESHNET_FLASH_WP,
        .quadhd_io_num = CONFIG_MESHNET_FLASH_HD,
    };

    const esp_flash_spi_device_config_t device_config = {
        .host_id = VSPI_HOST,
        .cs_id = 0,
        .cs_io_num = CONFIG_MESHNET_FLASH_CS,
